...
export function ModelBuilderPage({ id }: { id: string }) {
  ...
  // Same lookups feed the params preview and RulesTab's combo-table cells.
  // Query definitions are the last saved ones; unsaved Tables-tab path edits must not refetch.
  const lookups = usePreviewLookups(m.draft ?? EMPTY_MODEL, {
    enabled: tab === "params" && !!m.draft,
    queryTables: m.savedQueryTables,
  });
    ...
    <div style={{ height: "100%", display: "flex", flexDirection: "column" }}>
      ...
      >
        <ObjectPageSection id="params" titleText={secTitle("Parameters", "params")}>
          <ParamsTab draft={draft} update={m.update} issues={allIssues} tables={m.tableCols}
        ...
        <ObjectPageSection id="rules" titleText={secTitle("Rules", "rules")}>
          <RulesTab draft={draft} update={m.update} issues={allIssues} lookups={lookups.data} tables={suggestTables} />
        </ObjectPageSection>
        <ObjectPageSection id="bom" titleText={secTitle("BOM", "bom")}>
          <BomTab draft={draft} update={m.update} issues={allIssues} tables={suggestTables} />
        </ObjectPageSection>
        ...
        <ObjectPageSection id="settings" titleText={secTitle("Settings", "settings")}>
          <SettingsTab draft={draft} update={m.update} issues={allIssues} tables={suggestTables}
