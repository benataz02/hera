...
import {
  BusyIndicator, Button, CheckBox, Form, FormGroup, FormItem, Icon, IllustratedMessage, Input,
...
/** The same rules masterdata.save enforces, so Save is disabled instead of failing server-side. */
...
export function MasterdataEditor({ id }: { id?: string }) {
  ...
  const save = useMutation(orpc.masterdata.save.mutationOptions({
    onSuccess: (r) => {
      setDirty(false);
      invalidate();
  ...
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column" }}>
      {/* Above the page, not in snappedContent: an error the expanded header hides reads as a Save
        ...
        titleArea={
          <ObjectPageTitle
            ...
            snappedContent={<>
              ...
              {issue && dirty ? <MessageStrip design="Critical" hideCloseButton>{issue}</MessageStrip> : null}</>
            }
            actionsBar={
              <Toolbar design="Transparent" accessibleName="Table actions">
                <ToolbarButton design="Emphasized" disabled={!!issue || !dirty || save.isPending} text={save.isPending ? "Saving…" : "Save"}
                  onClick={() => save.mutate(
                    d.kind === "table"
          ...
          <Form {...PAIRS}>
            <FormItem labelContent={<Label required>Name</Label>}>
              <Input value={d.name} placeholder="prices" required
                valueState={dirty && !d.name.trim() ? "Negative" : "None"}
                      ...
                      <TableCell>
                        <div style={{ ...CELL, width: "100%" }}>
                          ...
                          <Input placeholder="key" value={c.key} style={{ width: "100%" }}
                            valueState={dirty && !c.key.trim() ? "Critical" : "None"}
              ...
              <FormGroup headerText="Clauses">
                ...
                <FormItem labelContent={<Label required>Entity set</Label>}>
                  <Input value={d.query.query.entitySet} placeholder="Items" required
                    valueState={dirty && !d.query.query.entitySet ? "Critical" : "None"}
