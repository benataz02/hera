...
export function buildAssistPrompt(model: ModelDef, propagated: Propagated, working: Working, ctx: Ctx): string {
  ...
  s.push("## How to work",
    ...
    "  Never select a positional candidate from memory: use the opaque candidateId",
    "  returned by calculate or by the latest selection.",
    ...
    "  user's picks on the configuration. Once calculate succeeds, configuration values",
