...
using live master data from the customer's own SAP B1 system. The same configurator can be opened
to the manufacturer's own customers as a self-service quote-request portal. HERA runs in the cloud
but reads and writes SAP through a small on-premise connector, so no ERP port is ever exposed to
the internet.
...
puts the rules in one place, keeps the data live from SAP, and makes the quote reproducible.
...
- **Lookup data** — option lists and price tables either maintained inside HERA or read live from
  SAP B1 / Beas, so material prices are never a stale copy.
...
- **Customer & item history** — the live SAP orders and quotations for this customer and/or this
  item code, fetched on demand, so the salesperson sees what this customer actually paid.
- **Similar configurations** — a ranked list of past production/sales records that resemble the
  configuration being entered, scored on the parameters the admin decided matter (exact match,
  numeric closeness, text contains, each with a weight). One click copies a past configuration's
...
### 6. Live SAP Business One data browser
An admin points HERA at the SAP B1 system and it **auto-discovers** every available entity
(business partners, items, orders, quotations, and any custom object). The admin chooses which
ones to expose and whether each is read-only or editable. Chosen entities appear as SAP-Fiori-style
list and detail pages inside HERA, with saved views: users can build their own column layouts,
filters and sort orders, and admins can publish shared views for the whole company.

### 7. Multi-company setup, roles and access
- Each customer company gets its own workspace on its own address (`company.hera-domain`), with its
  own data, users and SAP connection.
- Sign-in by email/password, Google or Microsoft. Users can belong to more than one workspace.
- Four levels of access: **owner/admin** (builds models, manages settings, invites), **member**
  (configures and quotes), **client** (portal only), and the **connector** itself.
...
### 8. On-premise connector
A small program the customer runs next to their SAP server. It dials out to HERA over ordinary
outbound HTTPS — **no inbound firewall rule, no VPN, no exposed SAP endpoint**. SAP credentials stay
on the customer's own machine and are never stored in the cloud. HERA shows whether the connector is
online, and a request that arrives while it is down fails with a clear message rather than silently
hanging. Writes to SAP are queued durably and are duplicate-proof: a network failure, a restart or a
double-start can never produce two documents in the ERP for one action.

---

## Not built yet (near-term)

- **Writing the finished quotation back into SAP B1** — the wizard's final "Create quote" step is
  present but disabled. Everything up to it works; the last write is the next milestone.
...
- **Deployment**: cloud (multi-tenant SaaS), one workspace per customer company, plus a lightweight
  on-premise connector the customer installs once next to their SAP server.
- **Prerequisite**: SAP Business One with the Service Layer enabled. Beas is optional.
...
- **Onboarding effort**: connector install and SAP entity selection is same-day; the real work is
  building the first product model with the customer's pricing rules — that is the consulting-shaped
  part of the engagement and the main switching cost once done.
...
- **Differentiators**: no-code configurator with a live test drive; live ERP data rather than
  imported copies; historic evidence in front of the salesperson at the moment of pricing; AI as a
  validated suggestion layer rather than an unaudited black box; and a security posture (outbound-only
  connector, credentials on-premise) that survives an IT review at a conservative manufacturer.
