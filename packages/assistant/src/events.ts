...
export const AssistantEventZ = z.discriminatedUnion("type", [
  z.strictObject({ ...envelope, type: z.literal("snapshot"),
    ...
    candidates: z.strictObject({ projectVersion: z.string().max(40), candidateCount: z.number().int().min(0), top: z.array(CandidateTopZ).max(5) }).optional(),
    selection: z.strictObject({ selections: z.array(SelectionRowZ).max(100) }).optional(),
  ...
  z.strictObject({ ...envelope, type: z.literal("selection"),
    selections: z.array(SelectionRowZ).max(100) }),
