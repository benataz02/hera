...
export const AssistantEventZ = z.discriminatedUnion("type", [
  z.strictObject({ ...envelope, type: z.literal("snapshot"),
    ...
    candidates: z.strictObject({ runId: z.uuid(), projectVersion: z.string().max(40), candidateCount: z.number().int().min(0), top: z.array(CandidateTopZ).max(5) }).optional(),
    selection: z.strictObject({ runId: z.uuid(), selections: z.array(SelectionRowZ).max(100) }).optional(),
